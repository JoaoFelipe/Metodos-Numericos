...
begin
  ...
  InitTest('A matriz [[3, 1, 1][2, 5, 1][2, 4, 9]] com vetor [3, 5, 9] e aproximação [0, 0, 0] deve gerar x=[0.7555, 0.5481, 0.4766] após 3 iterações e retornar 0 por convergir, mas não chegar ao valor proximo');
  //F=[[0, -0.33333, -0.33333][-0.4, 0, -0.2][-0.22222, -0.44444, 0]] e d=[1, 1, 1]
  //x=[0.5904, 0.6476, 0.5809]
  Matriz := NovaMatriz('[[3, 1, 1][2, 5, 1][2, 4, 9]]');
  AproximacaoInicial := NovoVetor('[0, 0, 0]');
  Vetor := NovoVetor('[3, 5, 9]');
  xResultado := NovoVetor('[0.7555, 0.5481, 0.4766]');
